...
export default function AssessmentStepper({ steps, currentIndex, completedIds, onJumpTo }) {
  return (
    <ol className="flex items-center gap-2 overflow-x-auto" aria-label="Assessment progress">
      {steps.map((step, idx) => {
        const completed = completedIds.includes(step.id);
        const active = idx === currentIndex;
        const enabled = completed || idx <= currentIndex;
        return (
